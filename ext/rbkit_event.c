...
VALUE rbkit_event_types_as_hash() {
  ...
  rb_hash_aset(events, ID2SYM(rb_intern("allocation_snapshot")), INT2FIX(allocation_snapshot));
...
rbkit_allocation_snapshot_event *new_rbkit_allocation_snapshot_event(rbkit_map_t *allocation_map) {
  rbkit_allocation_snapshot_event *event = malloc(sizeof(rbkit_allocation_snapshot_event));
  rbkit_event_header *header = (rbkit_event_header *)event;
  event->allocation_map = allocation_map;
  header->event_type = allocation_snapshot;
